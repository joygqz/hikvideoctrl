/**
 * 错误码枚举，用于 `switch` 分支判断与日志聚合。
 *
 * - `SDK_NOT_FOUND`         未检测到 `window.WebVideoCtrl`（多半 `webVideoCtrl.js` 未加载）
 * - `SDK_METHOD_MISSING`    SDK 目标方法不存在（常见于版本不匹配）
 ...
 * 海康封装库统一错误类型。
 *
 * 优先用 `code` 做分支判断，避免依赖 `message` 文案；原始错误经由 `cause` 透传（ES2022）。
 */
export class HikError extends Error {
  ...
  constructor(
    ...
    // 修复跨编译目标的原型链，保证 `instanceof HikError` 始终成立
    Object.setPrototypeOf(this, new.target.prototype)
  }
}

/** 将任意值收敛为 `HikError`，便于在 `catch` 中统一重抛。 */
